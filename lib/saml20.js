...
exports.create = function(options, callback) {
  ...
  if (options.lifetimeInSeconds && !options.subjectConfirmationKey) {
  ...
  if (options.subjectConfirmationKey) {
    confirmationData[0].setAttribute('xsi:type', 'saml2:KeyInfoConfirmationDataType');
    const keyInfoRaw = '<ds:KeyInfo xmlns:ds="http://www.w3.org/2000/09/xmldsig#"><ds:X509Data><ds:X509Certificate>' + cert + '</ds:X509Certificate></ds:X509Data></ds:KeyInfo>';
    const keyInfo = new Parser().parseFromString(keyInfoRaw);
    confirmationData[0].appendChild(keyInfo);
  }
